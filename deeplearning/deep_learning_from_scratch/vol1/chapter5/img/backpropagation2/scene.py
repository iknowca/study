...
class main(Scene):
    def construct(self):
        title = Text("Backpropagation").scale(1.5).shift(UP*3)
        self.add(title)
        
        node = Circle()
        node_label = MathTex("f").move_to(node.get_center())
        ...
        backward0_label = MathTex(r"E\frac{\partial y}{\partial x}").next_to(backward_arrow_0, DOWN).set_color(BLUE)
        ...
        backward1_label = MathTex(r"E").next_to(backward_arrow_1, DOWN).set_color(BLUE)
