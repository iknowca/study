...
class main(Scene):
    def construct(self):
        title = Text("Backpropagation - sigmoid").scale(1.5).shift(UP*3)
        self.add(title)
        
        node = Circle()
        node_label = Text("sigmoid").scale(0.8).move_to(node.get_center())
        ...
        backward0_label = MathTex(r"\frac{\partial L}{\partial y}y^2\exp(-x)").next_to(backward_arrow_0, DOWN).set_color(BLUE)
        backward0_label2 = MathTex(r"=\frac{\partial L}{\partial y}y(1-y)").next_to(backward0_label, DOWN).set_color(BLUE)
        self.add(forward_arrow_0, backward_arrow_0)
        self.add(forward0_label, backward0_label, backward0_label2)
